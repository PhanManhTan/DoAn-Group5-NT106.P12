...
namespace Server
{
    public partial class Server : Form
    {
        ...
        private void Listen()
        {
            try
            {
                while (true)
                {
                    ...
                    // MessageBox.Show("The client is connected to the server");
                }
            }
            catch (Exception ex)
            {
        ...
        void Finish(User user, Packet request) // Hàm xử lý khi có người thắng
        {
            Room room = FindRoom(request.RoomID);   // tìm phòng theo RoomID trong packet
        ...
        void AddInfo(User user, Packet request)  // Hàm gán các thông tin của packet nhận được vào các user
        {
        ...
        void UpdateUserInRoom(User currUser, Packet request) // hàm cập nhật cho các user khi có 1 user mới vào phòng
        {
